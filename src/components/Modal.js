            ...
            <div className="max-w-[600px] absolute top-1/2 left-1/2 z-20 bg-white rounded-[8px] transform -translate-x-1/2 -translate-y-1/2 p-5">
                ...
                <div>
                    ...
                    <a href={portfolio.siteUrl} target="_blank" rel="noopener noreferrer">
                        <button className='bg-orange-300 text-black py-2 px-4 my-8 rounded-[8px] font-[500] hover:bg-orange-300 ease-in duration-300'>Project Link</button>
                    </a>
                </div>

                <button onClick={() => setShowModal(false)}
                    className='w-[1.8rem] h-[1.8rem] bg-orange-300 absolute top-[1.7rem] right-[1.7rem] text-[25px] flex items-center justify-center rounded-[3px] leading-0 cursor-pointer'>&times;</button>
            </div>
        </div>
    )
}
